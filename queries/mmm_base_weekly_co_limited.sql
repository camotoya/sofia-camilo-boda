-- MMM base weekly for Habi Colombia - Google Meridian (sin tabla problemática)
...
-- Leads calificados por semana (simplificado sin JOIN problemático)
leads_weekly AS (
  SELECT
  ...
  WHERE
    ...
    -- Filtro simplificado sin el JOIN problemático
    AND (g.campana_mercadeo IS NOT NULL AND g.campana_mercadeo != '')
  GROUP BY 1
),

-- Inversión por canales (sin JOIN problemático)
inversion_weekly AS (
  SELECT
    CAST(DATE_TRUNC(i.date, WEEK(MONDAY)) as DATE) AS week_start,
    i.campana AS m_mkt_channel_small,
